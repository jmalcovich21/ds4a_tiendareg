...
def topleft_callbacks(app, df):
    ...
    def get_topleft_figure(select_product, select_marca, select_ciudad, select_zona, slider_pos, n_clicks):
        if select_product is None or select_marca is None or select_ciudad is None or select_zona is None:
            raise PreventUpdate
        else:
            if   slider_pos == 1:
                data = df.copy()
                ...
                df_filter_aux = df_filter.groupby(['DescripcionLargaProducto', 'Estrato', 'Month'])[
                ...
                df_filter_aux = df_filter_aux.groupby(['DescripcionLargaProducto', 'Estrato'])[
                ...
                for i in range(contador):
                    ...
                    try:
                        ...
                        if Indicador == 0:
                            model1 = auto_arima(serie, test='adf', error_action='ignore', suppress_warnings=True,
                                                stepwise=True,
                                                seasonal=False)
                        ...
                        desagregado_original = pd.DataFrame(
                            np.repeat(original.values / 7, np.repeat(7, 12, axis=0), axis=0))
                        desagregado_original.set_index(
                            pd.date_range(start=original.index[0], freq="D", periods=desagregado_original.size),
                            inplace=True)
                        ...
                        desagregado = pd.DataFrame(
                            np.repeat(prediccion.values / 7, np.repeat(7, len(prediccion), axis=0), axis=0))
                        desagregado.set_index(
                            pd.date_range(start=prediccion.index[0], freq="D", periods=desagregado.size),
                            inplace=True)
                ...
                mensual_grafico["Date"] = pd.to_datetime(
                    dict(year=mensual_grafico.Year, month=mensual_grafico.Month, day=1))
                ...
                best = best.groupby(['DescripcionLargaProducto', 'Estrato'])['Unidades'].median().reset_index(
                    name='Median').sort_values(['Median'], ascending=False)
                best['market share percentage by median'] = best['Median'] * 100 / best['Median'].sum()
                best['market share percentage by median'] = best['market share percentage by median'].apply(
                    lambda x: round(x, 2))

                fig = px.bar(best.head(10),
                                x="Median",
                                y="DescripcionLargaProducto",
                                color="Median",
                                orientation="h",
                                labels={'Median': 'Monthly Sales (median)', 'DescripcionLargaProducto': ''},
                ...
                fig.update_layout(title_text='BEST SELLING PRODUCTS - TOP 10',
                                     template='plotly_dark',
